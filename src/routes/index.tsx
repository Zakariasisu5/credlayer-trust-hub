...
function LandingPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Nav */}
      <header className="border-b border-border/60 backdrop-blur-md bg-background/60 fixed top-0 left-0 right-0 z-40">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <Link to="/" className="flex items-center gap-2">
            <img src={logoUrl} alt="CredLayer" className="h-8" />
          </Link>
          <nav className="hidden md:flex items-center gap-6 text-sm text-muted-foreground">
            <a href="#how-it-works" className="hover:text-foreground transition">How it works</a>
            <a href="#terminal3" className="hover:text-foreground transition">Terminal 3</a>
      ...
      {/* Terminal 3 */}
      <section id="terminal3" className="container mx-auto px-4 py-20">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-card/50 px-3 py-1 text-xs text-muted-foreground mb-4">
          ...
          <p className="mt-3 text-muted-foreground max-w-2xl mx-auto">
            CredLayer integrates Terminal 3 to issue, revoke, and verify
            on-chain reputation as W3C Verifiable Credentials — with explicit,
            scoped permissions for every AI agent acting on your behalf.
          </p>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          {[
            {
              ...
              body: "Grant read, write, or restricted scopes per AI agent. Restricted actions like transfers always require per-transaction signed approval.",
            },
            {
              icon: BadgeCheck,
              title: "Verifiable credentials",
              body: "Mint W3C VCs that embed your trust score, risk level, and confidence — signed by Terminal 3 when configured, locally otherwise.",
            },
            {
              icon: ScanLine,
              title: "Instant verification",
              body: "Anyone can verify a CredLayer credential by ID. Revocation and expiry are reflected in real time.",
            },
          ].map((step) => (
            <div key={step.title} className="glass-card rounded-xl p-6">
              <div className="h-10 w-10 rounded-lg bg-primary/10 grid place-items-center mb-4">
        ...
        <div className="mt-10 flex flex-col sm:flex-row justify-center gap-3">
          <Button asChild size="lg" variant="outline">
            <Link to="/verify" className="gap-2">
              <ScanLine className="h-4 w-4" /> Verify a credential
            </Link>
          </Button>
          <ConnectWalletButton size="lg" />
        </div>
        ...
        <div className="container mx-auto px-4 py-10 grid gap-8 md:grid-cols-3">
          <div>
            <div className="flex items-center gap-2">
              <img src={iconUrl} alt="CredLayer" className="h-7 w-7" />
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              ...
              <li><a href="#terminal3" className="hover:text-foreground">Terminal 3</a></li>
              <li><a href="#features" className="hover:text-foreground">Features</a></li>
              <li><Link to="/verify" className="hover:text-foreground">Verify credential</Link></li>
